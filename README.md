...
| `KEY_B DOWN` | クイック起動ランチャーを開く／閉じる |
| `KEY_C DOWN` | 次のデスクトップへ |
| `KEY_X UP` 等 | 受信するが無視（UDP のためパケット欠落あり得る） |

### クイック起動ランチャー

KEY_B（センターペダル）を押すと、**クイック起動（Quick Launch）フォルダ**の内容をタイル表示したランチャーがプライマリディスプレイの中央に開きます。もう一度押すか、ウィンドウ外をクリックすると閉じます。

クイック起動フォルダは Windows Vista 以降のタスクバーでは使われなくなりましたが、ファイルシステム上には残っています。このランチャー機能のために再利用できます。

#### フォルダを開く方法

`Win + R`（ファイル名を指定して実行）に以下を入力して開きます。

```
shell:Quick Launch
```

ここに `.lnk`（ショートカット）・`.exe`・`.rdp` など任意のファイルを置くとランチャーに表示されます。ファイルはすべて Windows の関連付けで開かれるため、拡張子を問いません。
